/// Static ISO 3166-1 country data used by the phone-number picker.
///
/// Mirrors the backend's `PhoneInput` component: the flag emoji is derived from
/// the ISO alpha-2 code (so no image assets are needed) and the widget submits a
/// single combined value like `+8801711223344`.
library;

/// A single country entry: ISO alpha-2 [iso], display [name] and telephone
/// [dialCode] (digits only, no leading `+`).
class Country {
  ...
  /// Unicode flag emoji derived from the alpha-2 code, e.g. `BD` → 🇧🇩.
...
/// Country directory + lookup helpers. Kept intentionally simple (a bundled
/// list) so the picker works fully offline.
abstract final class Countries {
  Countries._();

  /// Default selection, matching the backend's `app.default_country`.
  static const String defaultIso = 'BD';

  /// Resolve a country by ISO alpha-2 code (case-insensitive), falling back to
  /// [defaultIso] and finally the first entry.
  static Country byIso(String iso) {
    final upper = iso.toUpperCase();
    for (final c in all) {
      if (c.iso == upper) return c;
    }
    return byIso(defaultIso == upper ? all.first.iso : defaultIso);
  }

  /// Split a stored `+<dial><local>` value into its country and local part by
  /// longest matching dial code. Used to pre-fill the picker when editing an
  /// existing number. Returns the default country + raw digits when no dial
  /// code matches.
  static ({Country country, String local}) split(String? value) {
    final digits = (value ?? '').replaceAll(RegExp(r'\D'), '');
    if (digits.isEmpty) {
      return (country: byIso(defaultIso), local: '');
    }
    Country? best;
    for (final c in all) {
      if (c.dialCode.isEmpty || !digits.startsWith(c.dialCode)) continue;
      if (best == null || c.dialCode.length > best.dialCode.length) best = c;
    }
    if (best != null) {
      return (country: best, local: digits.substring(best.dialCode.length));
    }
    return (country: byIso(defaultIso), local: digits);
  }

  /// Case-insensitive search over name, ISO code and dial code.
  static List<Country> search(String query) {
    ...
    return all
        .where((c) =>
            ...
            c.dialCode.contains(normalized))
        .toList();
  }

  /// Full country list, sorted by name (see bottom of file).
